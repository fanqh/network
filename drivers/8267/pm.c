...
#define PM_LONG_SUSPEND_THRESH   0x0ff00000
...
__attribute__((section(".ram_code"))) int PM_LowPwrEnter(int DeepSleep, int WakeupSrc, unsigned long WakeupTick)
{
    ...
    unsigned short sys_ticks_per_32k_cycle = cali_32k_cnt/16;
    unsigned short half_sys_ticks_per_32k_cycle = sys_ticks_per_32k_cycle/2;
...
//        anareg2c &= 0xfc;
    ...
    WriteAnalogReg(ANA_REG_PM_PWDOWN_MASK, (DeepSleep ? 0xfe : 0x5e) | anareg2c);
    ...
    unsigned int m = span % cali_32k_cnt;
    span = M*16 + (m+half_sys_ticks_per_32k_cycle)/sys_ticks_per_32k_cycle; //with round operation
    ...
    M = span / 16;
    m = span % 16;
    span = M*cali_32k_cnt + m*sys_ticks_per_32k_cycle;
    tick_cur += span;
    ...
    return anareg44;
#else
    unsigned short tick_32k_calib = REG_PM_SYSTIMER_32K_CAL_CNT;
    unsigned short tick_32k_halfCalib = tick_32k_calib>>1;

    unsigned char long_suspend = 0;
    unsigned int span = (unsigned int)(WakeupTick - ClockTime());

    if(WakeupSrc & WAKEUP_SRC_TIMER){
        if (span > PM_SLEEP_DURATION_MAX) { //BIT(31)+BIT(30)   3/4 cylce of 32bit
            return  ReadAnalogReg(ANA_REG_PM_WAKEUP_STATUS) & 0x0f;
        }
        else if (span < PM_SLEEP_DURATION_MIN*TickPerUs) {// 0 us base
            unsigned long t = ClockTime();
            WriteAnalogReg(ANA_REG_PM_WAKEUP_STATUS, 0x0f); //clear all status
            unsigned char st;
            do {
            ...
            if (span > PM_LONG_SUSPEND_THRESH) {  //BIT(28) = 0x10000000   16M:16S; 32M:8S  48M: 5.5S
                long_suspend = 1;
            }
        }
    }
    ...
    ////////// disable IRQ //////////////////////////////////////////
    unsigned char r = REG_PM_IRQ_EN;
    REG_PM_IRQ_EN = 0;
    unsigned int tick_cur = ClockTime();
    unsigned int tick_32k_cur = PM_Get32kTick();
    unsigned int tick_wakeup_reset = WakeupTick - PM_WAKE_UP_MARGIN * TickPerUs;
    ...
    WriteAnalogReg(ANA_REG_PM_PWDOWN_MASK, (DeepSleep ? 0xfe : 0x5e) | rc32k_pd);
    WriteAnalogReg(ANA_REG_PM_XTAL_CTRL, ReadAnalogReg(ANA_REG_PM_XTAL_CTRL) & 0xdf); 

    span = (PM_DC_RESET_DURATION * TickPerUs * 16 + tick_32k_halfCalib)/tick_32k_calib;
    ...
    WriteAnalogReg(ANA_REG_PM_DCDC_DELAY_TIME, SWAP_BIT0_BIT6(rst_cycle));
    REG_PM_SYSTIMER_CTRL = 0x2c;
    REG_PM_SYSTIMER_32K_TICK = tick_reset;
    REG_PM_SYSTIMER_TRIG = BIT(3);
    while (REG_PM_SYSTIMER_TRIG & BIT(3)); 
    WriteAnalogReg(ANA_REG_PM_WAKEUP_STATUS, 0x0f); 
    ...
    if(long_suspend){
        tick_cur += (unsigned int)(PM_Get32kTick() - tick_32k_cur) / 16 * tick_32k_calib;
    }
    else{
        tick_cur += (unsigned int)(PM_Get32kTick() - tick_32k_cur) * tick_32k_calib / 16;        // current clock
    }
    ...
    REG_PM_SYSTIMER_CTRL = 0x00;
    REG_PM_SYSTIMER_CTRL = 0x92; //auto mode and enable 32K rc calibration
    REG_PM_SYSTIMER_TRIG = BIT(0); //enable system timer
